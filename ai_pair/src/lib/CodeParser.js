...
function showDiff(originalPath, newPath, fileLabel) {
    try {
        ...
        if (meaningfulDiff) {
            ...
            logger.debug(`No meaningful changes detected for ${fileLabel}: ${originalPath}`);
            return false;
        }
    } catch (diffError) {
        if (diffError.stdout) {
...
function parseAndApplyGeneratedCode(rootDir, tmpDir, extension, generatedCode) {
    ...
    if (codeBlocks.length === 0) {
        console.log('\nNo code changes recommended by AI');
    ...
    logger.info(`Summary: ${filesChanged} source files changed, ${filesAdded} files added, ${nonSourceFilesChanged} non-source files changed, ${noMeaningfulDiffCount} files had no meaningful diff.`);
